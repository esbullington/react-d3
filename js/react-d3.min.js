...
for(key in objB)if(objB.hasOwnProperty(key)&&!objA.hasOwnProperty(key))return!1;return!0}module.exports=shallowEqual},{}],143:[function(require,module){"use strict";function shouldUpdateReactComponent(prevElement,nextElement){return prevElement&&nextElement&&prevElement.type===nextElement.type&&prevElement.key===nextElement.key&&prevElement._owner===nextElement._owner?!0:!1}module.exports=shouldUpdateReactComponent},{}],144:[function(require,module){function toArray(obj){var length=obj.length;if(invariant(!Array.isArray(obj)&&("object"==typeof obj||"function"==typeof obj)),invariant("number"==typeof length),invariant(0===length||length-1 in obj),obj.hasOwnProperty)try{return Array.prototype.slice.call(obj)}catch(e){}for(var ret=Array(length),ii=0;length>ii;ii++)ret[ii]=obj[ii];return ret}var invariant=require("./invariant");module.exports=toArray},{"./invariant":127}],145:[function(require,module){"use strict";function userProvidedKeyEscaper(match){return userProvidedKeyEscaperLookup[match]}function getComponentKey(component,index){return component&&null!=component.key?wrapUserProvidedKey(component.key):index.toString(36)}function escapeUserProvidedKey(text){return(""+text).replace(userProvidedKeyEscapeRegex,userProvidedKeyEscaper)}function wrapUserProvidedKey(key){return"$"+escapeUserProvidedKey(key)}function traverseAllChildren(children,callback,traverseContext){return null==children?0:traverseAllChildrenImpl(children,"",0,callback,traverseContext)}var ReactElement=require("./ReactElement"),ReactInstanceHandles=require("./ReactInstanceHandles"),invariant=require("./invariant"),SEPARATOR=ReactInstanceHandles.SEPARATOR,SUBSEPARATOR=":",userProvidedKeyEscaperLookup={"=":"=0",".":"=1",":":"=2"},userProvidedKeyEscapeRegex=/[=.:]/g,traverseAllChildrenImpl=function(children,nameSoFar,indexSoFar,callback,traverseContext){var nextName,nextIndex,subtreeCount=0;if(Array.isArray(children))for(var i=0;i<children.length;i++){var child=children[i];nextName=nameSoFar+(nameSoFar?SUBSEPARATOR:SEPARATOR)+getComponentKey(child,i),nextIndex=indexSoFar+subtreeCount,subtreeCount+=traverseAllChildrenImpl(child,nextName,nextIndex,callback,traverseContext)}else{var type=typeof children,isOnlyChild=""===nameSoFar,storageName=isOnlyChild?SEPARATOR+getComponentKey(children,0):nameSoFar;if(null==children||"boolean"===type)callback(traverseContext,null,storageName,indexSoFar),subtreeCount=1;else if("string"===type||"number"===type||ReactElement.isValidElement(children))callback(traverseContext,children,storageName,indexSoFar),subtreeCount=1;else if("object"===type){invariant(!children||1!==children.nodeType);for(var key in children)children.hasOwnProperty(key)&&(nextName=nameSoFar+(nameSoFar?SUBSEPARATOR:SEPARATOR)+wrapUserProvidedKey(key)+SUBSEPARATOR+getComponentKey(children[key],0),nextIndex=indexSoFar+subtreeCount,subtreeCount+=traverseAllChildrenImpl(children[key],nextName,nextIndex,callback,traverseContext))}}return subtreeCount};module.exports=traverseAllChildren},{"./ReactElement":53,"./ReactInstanceHandles":61,"./invariant":127}],146:[function(require,module){"use strict";var emptyFunction=require("./emptyFunction"),warning=emptyFunction;module.exports=warning},{"./emptyFunction":108}],147:[function(require,module){module.exports=require("./lib/React")},{"./lib/React":29}],148:[function(require,module){module.exports={name:"react-d3",version:"0.0.5",description:"ReactJS charts using d3",main:"src/index.js",devDependencies:{browserify:"~6.2.0",chai:"^1.9.1",envify:"~3.0.0","highlight.js":"^8.3.0",karma:"^0.12.21","karma-bro":"^0.6.2","karma-chai":"^0.1.0","karma-chai-plugins":"^0.2.3","karma-chrome-launcher":"^0.1.4","karma-firefox-launcher":"^0.1.3","karma-jasmine":"^0.1.5","karma-mocha":"^0.1.9","karma-phantomjs-launcher":"^0.1.4",mocha:"^1.21.4","react-tools":"^0.12.0",reactify:"^0.15.2",uglifyjs:"^2.3.6",watchify:"^2.1.0"},dependencies:{d3:"^3.4.13",express:"^4.10.1",react:"^0.12.0",jsdom:"1.0.0"},scripts:{webpublish:"git subtree push --prefix dist origin gh-pages",start:"watchify -o dist/js/main.js examples/main.js & nodemon server.js",npmbuild:"NODE_ENV=production browserify --standalone react-d3 -o react-d3.js ./src/index.js",browserbuild:"NODE_ENV=production browserify --standalone react-d3 ./src/index.js | uglifyjs -c > react-d3.min.js",test:"./node_modules/karma/bin/karma start karma.conf.js"},author:"Eric S. Bullington",license:"MIT",repository:{type:"git",url:"https://github.com/esbullington/react-d3-charts.git"},browserify:{transform:["reactify","envify"]}}},{}],149:[function(require,module,exports){var React=require("react");window.React=React;var d3=(require("../package.json"),require("d3")),Chart=require("./common").Chart,XAxis=React.createClass({displayName:"XAxis",componentWillReceiveProps:function(props){var unit=props.xAxisTickInterval.unit,interval=props.xAxisTickInterval.interval,xAxis=d3.svg.axis().ticks(d3.time[unit],interval).scale(props.xScale).orient("bottom"),node=this.refs.xaxis.getDOMNode();d3.select(node).attr("class","x axis").call(xAxis),d3.select(".x.axis").selectAll("line").attr("shape-rendering","crispEdges").attr("stroke","#000"),d3.select(".x.axis").select("path").attr("shape-rendering","crispEdges").attr("fill","none").attr("stroke","#000")},render:function(){var t="translate(0,"+this.props.height+")";return React.createElement("g",{ref:"xaxis",className:"x axis",transform:t})}}),YAxis=React.createClass({displayName:"YAxis",componentWillReceiveProps:function(props){var yAxis=d3.svg.axis().ticks(props.yAxisTickCount).scale(props.yScale).orient("left"),node=this.refs.yaxis.getDOMNode();d3.select(node).attr("class","y axis").call(yAxis),d3.select(".y.axis").selectAll("line").attr("shape-rendering","crispEdges").attr("stroke","#000"),d3.select(".y.axis").select("path").attr("shape-rendering","crispEdges").attr("fill","none").attr("stroke","#000")},render:function(){return React.createElement("g",{ref:"yaxis",className:"y axis"})}}),Area=React.createClass({displayName:"Area",propTypes:{path:React.PropTypes.string,fill:React.PropTypes.string,height:React.PropTypes.number,width:React.PropTypes.number},getDefaultProps:function(){return{fill:"steelblue"}},render:function(){return React.createElement("path",{className:"area",d:this.props.path,fill:this.props.fill})}}),DataSeries=React.createClass({displayName:"DataSeries",render:function(){var props=this.props,area=d3.svg.area().x(function(d){return props.xScale(d.date)}).y0(this.props.height).y1(function(d){return props.yScale(d.value)}),path=area(this.props.data);return React.createElement(Area,{path:path})}}),AreaChart=React.createClass({displayName:"AreaChart",propTypes:{data:React.PropTypes.array,yAxisTickCount:React.PropTypes.number,xAxisTickInterval:React.PropTypes.object,width:React.PropTypes.number,height:React.PropTypes.number},getDefaultProps:function(){return{data:[],yAxisTickCount:4,xAxisTickInterval:{unit:"years",interval:1},width:400,height:200}},render:function(){var props=this.props,xScale=d3.time.scale().range([0,props.width]);xScale.domain(d3.extent(props.data,function(d){return d.date}));var yScale=d3.scale.linear().range([props.height,0]);yScale.domain([0,d3.max(props.data,function(d){return d.value})]);var margin={top:20,right:20,bottom:30,left:50},trans="translate("+margin.left+","+margin.top+")";return React.createElement(Chart,{ref:"chart",width:this.props.width+margin.left+margin.right,height:this.props.height+margin.top+margin.bottom},React.createElement("g",{transform:trans},React.createElement(DataSeries,{xScale:xScale,yScale:yScale,data:this.props.data,width:this.props.width,height:this.props.height}),React.createElement(XAxis,{xScale:xScale,xAxisTickInterval:this.props.xAxisTickInterval,margin:margin,width:this.props.width,height:this.props.height}),React.createElement(YAxis,{yScale:yScale,margin:margin,yAxisTickCount:this.props.yAxisTickCount,width:this.props.width,height:this.props.height})))}});exports.AreaChart=AreaChart},{"../package.json":148,"./common":151,d3:1,react:147}],150:[function(require,module,exports){var React=require("react"),d3=(require("../package.json"),require("d3")),Chart=require("./common").Chart,Bar=React.createClass({displayName:"Bar",propTypes:{fill:React.PropTypes.string,width:React.PropTypes.number,height:React.PropTypes.number,offset:React.PropTypes.number},getDefaultProps:function(){return{offset:0}},render:function(){return React.createElement("rect",{fill:this.props.fill,width:this.props.width,height:this.props.height,x:this.props.offset,y:this.props.availableHeight-this.props.height})}}),DataSeries=React.createClass({displayName:"DataSeries",propTypes:{fill:React.PropTypes.string,title:React.PropTypes.string,padding:React.PropTypes.number,width:React.PropTypes.number,height:React.PropTypes.number,offset:React.PropTypes.number},getDefaultProps:function(){return{padding:.1,data:[]}},render:function(){var props=this.props,yScale=d3.scale.linear().domain([0,d3.max(this.props.data)]).range([0,this.props.height]),xScale=d3.scale.ordinal().domain(d3.range(this.props.data.length)).rangeRoundBands([0,this.props.width],this.props.padding),bars=this.props.data.map(function(point,i){return React.createElement(Bar,{height:yScale(point),width:xScale.rangeBand(),offset:xScale(i),availableHeight:props.height,fill:props.fill,key:i})});return React.createElement("g",null,bars)}}),BarChart=React.createClass({displayName:"BarChart",render:function(){return React.createElement(Chart,{width:this.props.width,height:this.props.height},React.createElement(DataSeries,{data:this.props.data,width:this.props.width,height:this.props.height,fill:"cornflowerblue"}))}});exports.BarChart=BarChart},{"../package.json":148,"./common":151,d3:1,react:147}],151:[function(require,module,exports){var React=require("react");exports.Chart=React.createClass({displayName:"Chart",render:function(){return React.createElement("svg",{width:this.props.width,height:this.props.height},this.props.children)}})},{react:147}],152:[function(require,module,exports){require("d3"),require("react");exports.BarChart=require("./barchart").BarChart,exports.LineChart=require("./linechart").LineChart,exports.PieChart=require("./piechart").PieChart,exports.AreaChart=require("./areachart").AreaChart},{"./areachart":149,"./barchart":150,"./linechart":153,"./piechart":154,d3:1,react:147}],153:[function(require,module,exports){var React=require("react");window.React=React;var d3=(require("../package.json"),require("d3")),Chart=require("./common").Chart,Line=React.createClass({displayName:"Line",propTypes:{strokeWidth:React.PropTypes.number,path:React.PropTypes.string,fill:React.PropTypes.string,stroke:React.PropTypes.string},getDefaultProps:function(){return{stroke:"#1f77b4",fill:"none"}},render:function(){return React.createElement("path",{d:this.props.path,stroke:this.props.stroke,fill:this.props.fill,strokeWidth:this.props.strokeWidth})}}),Circle=React.createClass({displayName:"Circle",propTypes:{cx:React.PropTypes.number,cy:React.PropTypes.number,r:React.PropTypes.number,fill:React.PropTypes.string},getDefaultProps:function(){return{fill:"#1f77b4"}},render:function(){return React.createElement("circle",{cx:this.props.cx,cy:this.props.cy,r:this.props.r,fill:this.props.fill})}}),DataSeries=React.createClass({displayName:"DataSeries",propTypes:{data:React.PropTypes.array,interpolate:React.PropTypes.string},getDefaultProps:function(){return{data:[],interpolate:"linear"}},render:function(){var self=this,interpolatePath=d3.svg.line().x(function(d){return self.props.xScale(d.x)}).y(function(d){return self.props.yScale(d.y)}).interpolate(this.props.interpolate);return React.createElement(Line,{path:interpolatePath(this.props.data)})}}),LineChart=React.createClass({displayName:"LineChart",propTypes:{pointRadius:React.PropTypes.number,width:React.PropTypes.number,height:React.PropTypes.number},getDefaultProps:function(){return{pointRadius:2,width:400,height:200}},render:function(){var data=this.props.data,maxY=({width:this.props.width,height:this.props.height},d3.max(data,function(d){return d.y})),maxX=d3.max(data,function(d){return d.x}),xScale=d3.scale.linear().domain([0,maxX]).range([0,this.props.width]),yScale=d3.scale.linear().domain([0,maxY]).range([this.props.height,0]),circles=[];return this.props.data.forEach(function(point,i){circles.push(React.createElement(Circle,{cx:xScale(point.x),cy:yScale(point.y),r:this.props.pointRadius,key:i}))}.bind(this)),React.createElement(Chart,{width:this.props.width,height:this.props.height},React.createElement(DataSeries,{xScale:xScale,yScale:yScale,data:this.props.data,width:this.props.width,height:this.props.height}),circles)}});exports.LineChart=LineChart},{"../package.json":148,"./common":151,d3:1,react:147}],154:[function(require,module,exports){var React=require("react"),d3=(require("../package.json"),require("d3")),Chart=require("./common").Chart,Arc=React.createClass({displayName:"Arc",propTypes:{fill:React.PropTypes.string,d:React.PropTypes.string,startAngle:React.PropTypes.number,endAngle:React.PropTypes.number,innerRadius:React.PropTypes.number,outerRadius:React.PropTypes.number},render:function(){var arc=d3.svg.arc().innerRadius(this.props.innerRadius).outerRadius(this.props.outerRadius).startAngle(this.props.startAngle).endAngle(this.props.endAngle);return React.createElement("path",{className:"arc",d:arc(),fill:this.props.fill})}}),DataSeries=React.createClass({displayName:"DataSeries",propTypes:{transform:React.PropTypes.string,data:React.PropTypes.array,innerRadius:React.PropTypes.number,radius:React.PropTypes.number},getDefaultProps:function(){return{innerRadius:0,data:[]}},render:function(){var props=this.props,pie=d3.layout.pie().sort(null),arcData=pie(props.data),color=d3.scale.category20c(),arcs=[];return arcData.forEach(function(arc,i){arcs.push(React.createElement(Arc,{startAngle:arc.startAngle,endAngle:arc.endAngle,outerRadius:props.radius,innerRadius:props.innerRadius,fill:color(i),key:i}))}),React.createElement("g",{transform:this.props.transform},arcs)}}),PieChart=React.createClass({displayName:"PieChart",propTypes:{radius:React.PropTypes.number,cx:React.PropTypes.number,cy:React.PropTypes.number},render:function(){var transform="translate("+(this.props.cx||this.props.width/2)+","+(this.props.cy||this.props.height/2)+")";return React.createElement(Chart,{className:"pie-chart",width:this.props.width,height:this.props.height},React.createElement(DataSeries,{transform:transform,data:this.props.data,width:this.props.width,height:this.props.height,radius:this.props.radius,innerRadius:this.props.innerRadius}))}});exports.PieChart=PieChart},{"../package.json":148,"./common":151,d3:1,react:147}]},{},[152])(152)});